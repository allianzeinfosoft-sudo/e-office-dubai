            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    <div class="container-fluid flex-grow-1 container-p-y">
                        ...
                        <form id="payrollForm" action="{{ route('payroll.batches.update', $batch->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="hot-card-main shadow">
                                <div class="sticky-payroll-header">
                                    <h5 class="mb-0 text-white">Edit Batch:
                                        {{ date("F", mktime(0, 0, 0, $batch->month, 10)) }} {{ $batch->year }}</h5>
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('payroll.batches.index') }}" class="btn btn-light btn-sm">
                                    ...
                                    @foreach($structures as $index => $structure)
                                        <li class="nav-item" role="presentation">
                                            <button class="nav-link {{ $index === 0 ? 'active' : '' }}"
                                                id="tab-btn-{{ $structure->id }}" data-bs-toggle="tab"
                                                data-bs-target="#tab-panel-{{ $structure->id }}" type="button" role="tab">
                                                {{ $structure->name }}
                                                <span
                                                    class="badge bg-label-secondary ms-1">{{ count($hotData[$structure->id] ?? []) }}
                                                    Emp</span>
                                            </button>
                                        </li>
                                    @endforeach
                                ...
                                <div class="tab-content" id="payrollTabContent">
                                    @foreach($structures as $index => $structure)
                                        <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}"
                                            id="tab-panel-{{ $structure->id }}" role="tabpanel">
                                ...
                                <div class="card-footer bg-light border-top p-4 text-center">
                                    <button type="submit" class="btn btn-dark btn-lg px-5 fw-bold shadow">
                                        <i class="ti ti-device-floppy me-1"></i> UPDATE PAYROLL DATA
                                    </button>
                                </div>
                            </div>
                        </form>
