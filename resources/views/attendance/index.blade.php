...
@section('content')
                    ...
                    <div class="card card-sm">
                      <div class="card-header">
                        <h4 class="card-title mb-1"> <i class="ti ti-user ti-sm"></i>
                          {{ ucfirst(Auth::user()->employee?->full_name ?? 'N/A') }} </h4>
                      </div>
                            ...
                            @if(isset($attendance) || isset($attendance_current))
                              @if($shiftType == 'night')
                                @if($attendance_current?->signin_date == date('Y-m-d') && in_array($attendance_current?->status, ['mark-in', 'custom', 'emergency']))
                                  <div class="badge bg-label-success p-3 w-100 mb-3 text-dark" id="last-punch-time"
                                    role="alert">
                                    Last Punch In Time: {{ date('d-m-Y', strtotime($attendance_current?->signin_date)) }}
                                  ...
                                  @if($isWeekOffToday || $isHolidayToday)
                                    @if(!$prevAttendance || !$prevAttendance->signout_time)
                                      <div class="badge bg-label-danger p-3 w-100 mb-3" id="last-punch-time" role="alert">
                                        <strong>Missed Mark-out Detected:</strong> Please contact admin to regularize yesterday's
                                        attendance.
                                      </div>
                                    @else
                                ...
                                @elseif($attendance?->signin_date == date('Y-m-d', strtotime('-1 day')) && in_array($attendance?->status, ['mark-in', 'custom', 'emergency']))
                                  ...
                                  <div class="badge bg-label-success p-3 w-100 mb-3 text-dark" id="last-punch-time"
                                    role="alert">
                                    Last Punch In Time: {{ date('d-m-Y', strtotime($attendance?->signin_date)) }}
                              ...
                              @else
                                ...
                                @if(isset($attendance))
                                  @if(in_array($attendance->status, ['mark-in', 'custom', 'emergency']))
                                    ...
                                    <div class="text-center">
                                      <button type="button" id="mark-out-btn" class="btn p-3 btn-danger w-100"> <i
                                          class="ti ti-arrow-big-left-lines ti-sm"></i> Mark-out </button>
                                    </div>
                    ...
                    <div class="card card-bg pt-3">
                      <div class="row g-4 p-3">
                        ...
                        <div class="col-12 col-md-6 col-xl-12 col-lg-12 pt-2">
                          <button type="button" class="btn p-3 btn-info w-100" onclick="customModal()">Custom <i
                        ...
                        <div class="col-12 col-md-6 col-xl-12 col-lg-12">
                          <button type="button" class="btn p-3 btn-warning w-100" onclick="emergencyModal()">Emergency <i
                        ...
                        <div class="col-6 col-md-6 col-xl-6 col-lg-6">
                          <button type="button" class="btn btn-primary w-100" onclick="wfh_attendance()">Work From
                            Home</button>
                        </div>

                        <div class="col-6 col-md-6 col-xl-6 col-lg-6">
                          <button type="button" class="btn btn-danger w-100" onclick="wos_attendance()">Work On
                            Site</button>
                        </div>
                        <!--/ work from home -->
        ...
        <form id="customMarkingForm" action="{{ route('attendance.custom-mark-in') }}" method="post">
          ...
          <div class="col-12 mb-3">
            ...
            <input type="time" id="signin_time" name="signin_time" class="form-control"
              value="{{ \Carbon\Carbon::now(config('app.timezone'))->format('H:i') }}" placeholder="Time" />
            <input type="hidden" id="signin_date" name="signin_date" class="form-control" value="{{ date('Y-m-d') }}"
              placeholder="Time" />
          </div>

          <div class="col-12 mb-3">
            <label for="signin_late_note" class="form-label">Reason</label>
            <textarea id="signin_late_note" name="signin_late_note" class="form-control" placeholder="Reason"
              rows="5"></textarea>
          </div>
        </form>
        <div class="col-sm-12 d-flex justify-content-end align-items-center gap-2">
          <button type="button" class="btn btn-label-secondary" data-bs-dismiss="offcanvas" aria-label="Close"> Close
          </button>
  ...
  <div class="offcanvas offcanvas-end w-45" data-bs-backdrop="static" tabindex="-1" id="emergencyMarkingOffcanvas"
    aria-labelledby="staticBackdropLabel">
    <div class="offcanvas-header bg-primary">
      <h5 class="offcanvas-title text-white" id="staticBackdropLabel"> <i class="ti ti-device-watch float-start fs-3"></i>
        Emergency Marking</h5>
      <button type="button" class="btn btn-danger offcanvas-close" data-bs-dismiss="offcanvas" aria-label="Close"><i
          class="fa fa-close"></i> </button>
    </div>
    <div class="offcanvas-body">
