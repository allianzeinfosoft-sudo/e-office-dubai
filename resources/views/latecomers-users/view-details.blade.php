...
@else
    <table class="table table-bordered table-striped table-hover" id="latecommer_details">
        <thead>
            <tr>
                <th>#</th>
                <th>Image</th>
        ...
        <tbody>
            @foreach($lateAttendances as $index => $entry)
                @if($entry->working_hours < '06:00:00')
                ...
                @else
                    <tr>
                        ...
                        <td><button type="button" class="btn btn-primary">{{ $entry->working_hours ?? '00:00:00' }}</button></td>
                    </tr>
                @endif
