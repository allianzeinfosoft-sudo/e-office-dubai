...
@endif -->
<form class="add-new-department pt-0 row g-2" method="post" action="{{ route('create.designation') }}"
  id="department-form" onsubmit="return false">
  @csrf
  ...
  <div class="col-sm-12">
    <label class="form-label" for="basicBranchname3">Branch Name
      ...
      <button type="button" class="btn btn-icon btn-xs btn-outline-primary waves-effect" data-bs-toggle="modal"
        data-bs-target="#addNewBranchModal">
  ...
  <div class="col-sm-12">
    <label class="form-label" for="department">Department
      <button type="button" class="btn btn-icon btn-xs btn-outline-primary waves-effect" data-bs-toggle="modal"
        data-bs-target="#addNewDepartmentModal">
      ...
      <div class="modal-body">
        ...
        <form id="add-branch-form" action="{{ route('branchs.store') }}" method="POST" class="row g-3">
          ...
          <div class="col-12">
            ...
            <div class="input-group input-group-merge">
              <input id="branch_name" name="branch_name" class="form-control" type="text"
                placeholder="Enter Branch Name" aria-describedby="branch_name" />
          ...
          <div class="col-12">
            ...
            <div class="input-group input-group-merge">
              <input id="branch_location" name="branch_location" class="form-control" type="text"
                placeholder="Enter Branch Location" aria-describedby="branch_location" />
              <span class="input-group-text cursor-pointer p-1" id="modalAddCard2"><span
                  class="card-type"></span></span>
            </div>
          </div>
  ...
  <div class="modal-dialog modal-dialog-centered1 modal-simple modal-add-new-cc">
    <div class="modal-content p-3 p-md-5">
      <div class="modal-body">
        ...
        <form id="add-department-form" action="{{ route('departments.store') }}" method="POST" class="row g-3">
          @csrf
          <div class="col-12">
            <label class="form-label w-100" for="modalAddCard">Branch Name</label>
          ...
          <div class="col-12">
            ...
            <div class="input-group input-group-merge">
              <input id="department_name" name="department_name" class="form-control" type="text"
                placeholder="Enter Department" aria-describedby="department_name" />
              <span class="input-group-text cursor-pointer p-1" id="modalAddCard2"><span
                  class="card-type"></span></span>
            </div>
          </div>
...
@push('js')
  <script>
    ...
    $(document).ready(function () {
      ...
      $('#addNewDepartmentModal').on('show.bs.modal', function () {

        let selectedBranch = $('#basicBranchname3').val();

        if (selectedBranch) {
          $('#branch_select').val(selectedBranch).trigger('change');
        }
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.data) {
            ...
            // 👉 Create option
            let newOption = new Option(response.data.branch, response.data.id, false, false);

            // 👉 Add to dropdown
            $('#basicBranchname3').append(newOption);

            // 👉 Select it
            $('#basicBranchname3').val(response.data.id).trigger('change');

            // 👉 Update department modal dropdown
            $('#branch_select').append(
              new Option(response.data.branch, response.data.id, false, false)
            ).val(response.data.id).trigger('change');
          }
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.data) {
            ...
            // 👉 Create option
            let newOption = new Option(response.data.department, response.data.id, false, false);
            ...
            // 👉 Update department modal dropdown
            $('#branch_select').append(
              new Option(response.data.department, response.data.id, false, false)
            ).val(response.data.id).trigger('change');
          }
