                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <div class="col-sm-12 d-flex justify-content-end mb-3">
                            <a class="btn add-new btn-primary" href="javascript:void(0);" onclick="openFeedbackOffcanvas()">
                                ...
                                <span>
                                    ...
                                    <span class="d-none d-sm-inline-block">Add New Feedback</span>
                                </span>
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-datatable table-responsive">
                            <table class="hover_effect datatables-basic datatables-feedback table border-top table-stripedc" id="datatables-feedback">
                                <thead>
                                    <tr>
                                        <th>S.No</th>
...
<div class="offcanvas offcanvas-end w-45" data-bs-backdrop="static" tabindex="-1" id="feedback_offcanvas" aria-labelledby="staticBackdropLabel">
    <div class="offcanvas-header bg-primary p-3">
        <span class="d-flex justify-content-between align-items-center gap-2">
            <i class="ti ti-file-plus fs-2 text-white"></i>
            <span id="offcanvas-title-container">
                <h5 class="offcanvas-title text-white" id="staticBackdropLabel"> Add Feedback</h5>
                <span class="text-white slogan">Create New Feedback</span>
            </span>
        </span>
...
<script>
    ...
    $(function() {
        var feedbackTable = $('.datatables-feedback'),
        select2 = $('.select2');
        if (feedbackTable.length) {
            ...
            feedbackTable.DataTable({
                ajax: {
                    type: "GET",
                    url: "{{ route('feedback.index') }}", // Fixed syntax
                    dataType: "json",
                    dataSrc: "data"
                },
                columns: [
                    ...
                    { data: 'created_date', title: 'Created Date'},
                    {
                        ...
                        render: function (data, type, row, full) {
                            const editUrl = "{{ route('feedback.edit', ':id') }}".replace(':id', row.id);
                            return `
                                <a href="javascript:void(0)" class="btn btn-sm btn-icon btn-primary view-feedback" title="view questions"  onclick="openFeedbackQuestionOffcanvas(${row.id})""><i class="ti ti-eye"></i></a>
                                <a href="javascript:void(0)" class="btn btn-sm btn-icon btn-primary edit-feedback" title="edit" onclick="openFeedbackOffcanvas(${row.id})"><i class="ti ti-edit"></i></a>
                                <a href="javascript:void(0)" class="btn btn-sm btn-icon btn-danger delete-feedback" title="delete" data-id="${row.id}"><i class="ti ti-trash"></i></a>`;
                        }
                    }
                ]
            });
        ...
        Swal.fire({
                ...
                if (result.isConfirmed) {
                        ...
                        success: function(response) {
                           ...
                           Swal.fire("Deleted!", response.message || "Feedback has been deleted.", "success").then(() => {
                                $('#datatables-feedback').DataTable().ajax.reload(); // Reload table
                            });
    ...
    if (targetId) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#feedback_title').val(data.name);
                $('#department_id').val(data.department).trigger('change');

                // Check if editing is allowed
                if (data.locked) {
                    $('#question-container').html(`<div class="alert alert-warning">This feedback is assigned to employees and cannot be edited.</div>`);
