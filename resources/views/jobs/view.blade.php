<div class="row">
    ...
    <div class="card overflow-hidden mb-4">
        <div class="card-body">
            <div class="d-flex  align-items-center justify-content-between">
                <span> <i class="ti ti-user ti-md"></i> {{ $job->createdBy->full_name }} </span>
                <span> <i class="ti ti-clock ti-md"></i> {{ date('d M Y', strtotime($job->created_at)) }}</span>
            </div>
            ...
            <div class="d-flex justify-content-between flex-wrap gap-3">
                <div class="article-info">
                    ...
                    <h5 class="">{{ $job->assignedTo?->full_name }}</h5>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body pb-0">
            <form method="POST" id="job_comments_form">
                @csrf
                <div class="row">
                    <div class="col-sm-12">
                        <label for="note_comment" class="form-label">Commets</label>
                        <textarea class="form-control" id="job_comment" name="job_comment" rows="3" placeholder="Enter Comment"></textarea>
                    </div>
            ...
            <ul class="timeline mt-4 mb-0" id="job_comments_list">
                @if($comments->count() > 0)
