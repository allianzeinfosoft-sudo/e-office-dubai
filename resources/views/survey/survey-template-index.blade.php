                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <div class="col-sm-12 d-flex justify-content-end mb-3">
                            <a class="btn add-new btn-primary" href="javascript:void(0);" onclick="openSurveyTemplateOffcanvas()">
                                ...
                                <span>
                                    ...
                                    <span class="d-none d-sm-inline-block">Add New Survey</span>
                                </span>
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-datatable table-responsive">
                            <table class="hover_effect datatables-basic datatables-survey-template table border-top table-stripedc" id="datatables-survey-template">
                                <thead>
                                    <tr>
                                        <th>S.No</th>
                                        <th>Survey Name</th>
...
<div class="offcanvas offcanvas-end w-45" data-bs-backdrop="static" tabindex="-1" id="survey_template_offcanvas" aria-labelledby="staticBackdropLabel">
    <div class="offcanvas-header bg-primary p-3">
        <span class="d-flex justify-content-between align-items-center gap-2">
            <i class="ti ti-file-plus fs-2 text-white"></i>
            <span id="offcanvas-title-container">
                <h5 class="offcanvas-title text-white" id="staticBackdropLabel"> Add Survey</h5>
                <span class="text-white slogan">Create New Survey</span>
            </span>
        </span>
...
<script>
    ...
    $(function() {
        var surveyTemplateTable = $('.datatables-survey-template'),
        select2 = $('.select2');
        if (surveyTemplateTable.length) {
            ...
            surveyTemplateTable.DataTable({
                ajax: {
                    type: "GET",
                    url: "{{ route('surveytemplate.index') }}", // Fixed syntax
                    dataType: "json",
                    dataSrc: "data"
                },
                columns: [
    ...
    $(document).on('click', '.delete-survey-template', function(e) {
        ...
        Swal.fire({
                ...
                if (result.isConfirmed) {
                        ...
                        $.ajax({
                        url: `/surveytemplate/${surveyTemplateId}`,
                        type: 'DELETE',
                        data: {
                        ...
                        success: function(response) {
                            ...
                            Swal.fire("Deleted!", "Survey has been deleted.", "success").then(() => {
                                $('#datatables-survey-template').DataTable().ajax.reload(); // Reload table
                            });
  ...
  function openSurveyQuestionOffcanvas(surveyTemplateId) {
...
function openSurveyTemplateOffcanvas(targetId = null) {
    $('#survey-template-form')[0].reset(); // Reset form
